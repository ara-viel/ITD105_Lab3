# classification/model_A_kfold.py
"""
Model A - Classification (Logistic Regression + K-Fold CV)
Dataset: Pima Indians Diabetes (expecting 9 columns or uses remote mirror)
Saves:
 - model file: classification_model_A_kfold.joblib
 - metrics file: classification_model_A_kfold_metrics.json
...
OUT_MODEL = Path(__file__).resolve().parent / "classification_model_A_kfold.joblib"
OUT_METRICS = Path(__file__).resolve().parent / "classification_model_A_kfold_metrics.json"
...
def main():
    ...
    print(f"Running {k}-Fold cross-validation... (this may take a few seconds)")
    acc_scores = cross_val_score(pipeline, X, y, cv=kf, scoring='accuracy', n_jobs=-1)

    # get predicted probabilities for log loss and AUC
    ...
    # confusion and classification report on whole dataset predicted by CV (approx)
